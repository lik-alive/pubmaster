<script>
	$(document).ready(function() {
		...
		var table = dt.DataTable( {
			...
			"processing": false,				
			"language": {
				'emptyTable': "<div style='text-align: center'>Докладов не обнаружено</div>",
			...
			{ 'name': 'id', "data": "ID_Conference", 'visible': false },
			...
			{ 'name': 'title', "data": "Title", 'class': 'sorting',
				"render": function (data, type, JsonResultRow, meta) {
					if ( type === 'sort' || type === 'type' ) {
						...
						var str = "<a class='article' href='" + SITE_URL + "/articles/view/?id=" + JsonResultRow['ID_Article'] + "'>" + JsonResultRow['Title'] + '</a><br/>';
						...
						str += '<span class="other">"'+ JsonResultRow['CTitle'] + '".<br/>'
						+ JsonResultRow['CCity'] + ', ' + JsonResultRow['CountryName'] + ', '
						+ revertDate(JsonResultRow['DateFrom']) + ' - ' + revertDate(JsonResultRow['DateTo']) + '</span>';
		...
		table.on('click', '.upload-action', function() {
			window.open(SITE_URL + '/articles/edit/?id=' + table.row($(this).closest('tr')).data().ID_Article, '_blank');
		});
		
		//Hide pagination list
		$('#conferencestable_wrapper .dataTables_length')[0].style.display='none';
...
<table id='conferencestable' class='mydataTable stripe hover'>
	<thead>
		<tr>
