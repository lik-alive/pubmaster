...
class FileManager {
	...
	//Handle files-drop
	addFiles(files) {
		if (!this.options.includes(FileManagerOptions.Multiple) && this.filesCount > 0) return;
	...
	embedFileInfo(file) {
		...
		var name = $("<div class='file-manager-fname'>" + file.name + "</div>");
		var length = $("<div class='file-manager-flength'>" + Math.round(file.size/1024/1024*100)/100 + ' MB' + "</div>");
		
		var closeButton = $(
			"<div class='file-manager-fclose'>" +
				"<i class='zmdi zmdi-close zmdi-action file-manager-fclose-icon'></i>" +
			"</div>");
